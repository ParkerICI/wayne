...
(def examples
  [{:text "Find relative intensity of fucosylated glycans in WHO grade 2,3 and 4 samples. "
    ...
    :vega-params {"blobWidth" 100 "blobSpace" 328}
    }
   ...
   {:text "Find mean intensity of EGFR expression on tumor cells in primary WHO grade 3 and 4 samples."
    :params {:universal {:dim :WHO_grade, :feature "EGFR", :filters {:Tumor_Diagnosis {"Astrocytoma" false, "GBM" false, "Oligodendroglioma" false}, :recurrence {"No" true}, :WHO_grade {"3" true, "4" true}}}, :heatmap2 {:dim :WHO_grade, :filter {:Tumor_Diagnosis {"Astrocytoma" false, "GBM" false, "Oligodendroglioma" false}, :recurrence {"No" true}, :WHO_grade {"3" true, "4" true}}}, :features {:feature-bio-feature-type nil, :feature-supertype "nonspatial", :scale "linear", :feature-broad_feature_type "Protein", :feature-feature_type "Tumor_Antigens_Intensity", :feature-feature_variable "EGFR"}}
    :vega-params {"blobWidth" 100 "blobSpace" 328}
    }
   ...
   {:text "Find mean intensity of CD14 expression in primary WHO grade 4 samples and compare between IDH mutant and wild type"
    ...
    :vega-params {"blobWidth" 100 "blobSpace" 277}
    }
   ])
...
(rf/reg-event-db
 ...
 (fn [db [_ text]]
   (let [example (u/some-thing #(= (:text %) text) examples)]
     (prn :recall text example)     
     (u/merge-recursive db example))))
